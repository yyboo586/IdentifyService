...
type IMQ interface {
	Subscribe(ctx context.Context, topic string, channel string, handler mqsdk.MessageHandler) error
	Publish(ctx context.Context, topic string, msg *mqsdk.Message) error
}
...
func NewMQ() *MQ {
	...
	producer, err := factory.NewProducer(nsqConfig)
	if err != nil {
...
func (m *MQ) Subscribe(ctx context.Context, topic string, channel string, handler mqsdk.MessageHandler) (err error) {
...
func (m *MQ) Publish(ctx context.Context, topic string, msg *mqsdk.Message) (err error) {
	err = m.producer.Publish(ctx, topic, msg)
	if err != nil {
		g.Log().Error(ctx, "publish failed", err)
	}
	return
}
