...
func (s *org) Get(ctx context.Context, id string) (out *model.Org, err error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, fmt.Errorf("%w, %v", model.ErrRecordNotFound, fmt.Errorf("org_id: %s", id))
		}
		return nil, fmt.Errorf("%w, %v", model.ErrServerInternal, err)
	}

	return s.convertEntityToLogics(&org), nil
}
...
// checked
func (s *org) IsEnabled(ctx context.Context, orgID string) (out *model.Org, err error) {
	out, err = s.Get(ctx, orgID)
	if err != nil {
		return nil, err
	}

	if out.Status != model.OrgStatusEnabled {
		return nil, fmt.Errorf("组织已被禁用")
	}
