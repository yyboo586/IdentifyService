...
func (c *orgService) CreateRole(ctx context.Context, req *system.RoleCreateReq) (res *system.RoleCreateRes, err error) {
	err = c.checkPermission(ctx, []model.PermissionPoint{model.RoleCreate})
	if err != nil {
		return nil, err
	}

	// 检查组织是否启用
	err = c.domainOrg.IsEnabled(ctx, req.OrgID)
	if err != nil {
		return nil, err
	}
	...
	req.PermissionPointIDs, err = c.domainPermission.FilterPermissionPointIDsByUserID(ctx, req.PermissionPointIDs, currentUserId)
	if err != nil {
		return nil, err
	}
	...
	err = g.DB().Transaction(ctx, func(ctx context.Context, tx gdb.TX) (err error) {
		...
		if len(req.PermissionPointIDs) > 0 {
			err = c.domainPermission.AssignRolePermissions(ctx, res.ID, req.PermissionPointIDs)
			if err != nil {
				return err
			}
		}
...
func (c *orgService) DeleteRole(ctx context.Context, req *system.RoleDeleteReq) (res *system.RoleDeleteRes, err error) {
	err = c.checkPermission(ctx, []model.PermissionPoint{model.RoleDelete})
	if err != nil {
		return nil, err
	}
...
func (c *orgService) EditRole(ctx context.Context, req *system.RoleEditReq) (res *system.RoleEditRes, err error) {
	err = c.checkPermission(ctx, []model.PermissionPoint{model.RoleEdit})
	if err != nil {
		return nil, err
	}
	...
	err = g.DB().Transaction(ctx, func(ctx context.Context, tx gdb.TX) (err error) {
		...
		if len(req.PermissionPointIDs) > 0 {
			err = c.domainPermission.AssignRolePermissions(ctx, req.RoleID, req.PermissionPointIDs)
			if err != nil {
				return err
			}
		}
...
func (c *orgService) GetRole(ctx context.Context, req *system.RoleGetReq) (res *system.RoleGetRes, err error) {
	err = c.checkPermission(ctx, []model.PermissionPoint{model.RoleView, model.RoleAssign})
	if err != nil {
		return nil, err
	}
	...
	_, err = c.domainPermission.GetPermissionPointsByRoleID(ctx, out.ID)
	if err != nil {
		return nil, err
	}
