	...
	err = g.DB().Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
		err = g.Try(ctx, func(ctx context.Context) {
			...
			data := map[string]interface{}{
				dao.SysAuthRule.Columns().Pid:       req.Pid,
				dao.SysAuthRule.Columns().Name:      req.Name,
				dao.SysAuthRule.Columns().Title:     req.Title,
				dao.SysAuthRule.Columns().Icon:      req.Icon,
				dao.SysAuthRule.Columns().Condition: req.Condition,
				dao.SysAuthRule.Columns().Remark:    req.Remark,
				dao.SysAuthRule.Columns().MenuType:  req.MenuType,
				dao.SysAuthRule.Columns().Weigh:     req.Weigh,
				dao.SysAuthRule.Columns().IsHide:    req.IsHide,
				dao.SysAuthRule.Columns().Path:      req.Path,
				dao.SysAuthRule.Columns().Component: req.Component,
				dao.SysAuthRule.Columns().IsLink:    req.IsLink,
				dao.SysAuthRule.Columns().IsIframe:  req.IsIframe,
				dao.SysAuthRule.Columns().IsCached:  req.IsCached,
				dao.SysAuthRule.Columns().Redirect:  req.Redirect,
				dao.SysAuthRule.Columns().IsAffix:   req.IsAffix,
				dao.SysAuthRule.Columns().LinkUrl:   req.LinkUrl,
			}
...
func (s *sSysAuthRule) GetMenuRoles(ctx context.Context, id uint) (roleIds []uint, err error) {
	err = g.Try(ctx, func(ctx context.Context) {
		enforcer, e := commonService.CasbinEnforcer(ctx)
		liberr.ErrIsNil(ctx, e)
		policies, _ := enforcer.GetFilteredNamedPolicy("p", 1, gconv.String(id))
