	...
	err := g.Try(ctx, func(ctx context.Context) {
		...
		var (
			...
			data = map[string]interface{}{
				dao.SysUserOnline.Columns().Uuid:       params.Uuid,
				dao.SysUserOnline.Columns().Token:      params.Token,
				dao.SysUserOnline.Columns().CreateTime: gtime.Now(),
				dao.SysUserOnline.Columns().UserName:   params.Username,
				dao.SysUserOnline.Columns().Ip:         params.Ip,
				dao.SysUserOnline.Columns().Explorer:   browser,
				dao.SysUserOnline.Columns().Os:         os,
			}
		)

		//查询是否已存在当前用户
		err := dao.SysUserOnline.Ctx(ctx).Fields(dao.SysUserOnline.Columns().Id).
			Where(dao.SysUserOnline.Columns().Token, data[dao.SysUserOnline.Columns().Token]).
			Scan(&info)
...
func (s *sSysUserOnline) CheckUserOnline(ctx context.Context) {
	...
	service.SysJobLog().Add(ctx, map[string]interface{}{
		dao.SysJobLog.Columns().TargetName: "checkUserOnline",
		dao.SysJobLog.Columns().CreatedAt:  gtime.Now(),
		dao.SysJobLog.Columns().Result:     "在线用户定时更新，执行成功",
	})
}
