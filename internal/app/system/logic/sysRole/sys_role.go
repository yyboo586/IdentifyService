...
func (s *sSysRole) AddRole(ctx context.Context, req *system.RoleAddReq) (err error) {
	err = g.DB().Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
		err = g.Try(ctx, func(ctx context.Context) {
			...
			roleId, e := dao.SysRole.Ctx(ctx).TX(tx).InsertAndGetId(map[string]interface{}{
				dao.SysRole.Columns().Pid:           req.Pid,
				dao.SysRole.Columns().Status:        1,
				dao.SysRole.Columns().ListOrder:     0,
				dao.SysRole.Columns().Name:          req.Name,
				dao.SysRole.Columns().Remark:        req.Remark,
				dao.SysRole.Columns().CreatedBy:     req.CreatedBy,
				dao.SysRole.Columns().EffectiveTime: model.EffectiveTimeInfo{},
			})
...
func (s *sSysRole) Get2(ctx context.Context, roleID uint) (res *model.RoleInfoRes, err error) {
	err = g.Try(ctx, func(ctx context.Context) {
		res = new(model.RoleInfoRes)
		err = dao.SysRole.Ctx(ctx).WherePri(roleID).Scan(&res.SysRole)
		liberr.ErrIsNil(ctx, err, "获取角色信息失败")
	})
	return
}

func (s *sSysRole) GetByName(ctx context.Context, roleName string) (res *model.RoleInfoRes, err error) {
	err = g.Try(ctx, func(ctx context.Context) {
		res = new(model.RoleInfoRes)
		err = dao.SysRole.Ctx(ctx).Where(dao.SysRole.Columns().Name, roleName).Scan(&res.SysRole)
		liberr.ErrIsNil(ctx, err, "获取角色信息失败")
	})
	return
}
...
func (s *sSysRole) EditRole(ctx context.Context, req *system.RoleEditReq) (err error) {
	err = g.DB().Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
		err = g.Try(ctx, func(ctx context.Context) {
			...
			_, e := dao.SysRole.Ctx(ctx).TX(tx).WherePri(req.Id).Data(map[string]interface{}{
				dao.SysRole.Columns().Pid:           req.Pid,
				dao.SysRole.Columns().Status:        req.Status,
				dao.SysRole.Columns().ListOrder:     req.ListOrder,
				dao.SysRole.Columns().Name:          req.Name,
				dao.SysRole.Columns().Remark:        req.Remark,
				dao.SysRole.Columns().EffectiveTime: gconv.String(req.EffectiveTimeInfo),
			}).Update()
...
func (s *sSysRole) RoleDataScope(ctx context.Context, req *system.DataScopeReq) error {
	err := g.DB().Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
		err := g.Try(ctx, func(ctx context.Context) {
			data := make([]map[string]interface{}, 0, len(req.AuthData))
			for _, v := range req.AuthData {
				if v.MenuId != 0 && v.Scope != 0 {
					data = append(data, map[string]interface{}{
						dao.SysRoleScope.Columns().RoleId:    req.RoleId,
						dao.SysRoleScope.Columns().MenuId:    v.MenuId,
						dao.SysRoleScope.Columns().DataScope: v.Scope,
						dao.SysRoleScope.Columns().DeptIds:   gconv.String(v.DeptIds),
					})
				}
			}
			//清除旧权限
...
func (s *sSysRole) refreshToken(ctx context.Context, roleId int64) {
	_ = g.Try(ctx, func(ctx context.Context) {
		enforcer, e := commonService.CasbinEnforcer(ctx)
		liberr.ErrIsNil(ctx, e)
		userRoleIds, _ := enforcer.GetFilteredGroupingPolicy(1, gconv.String(roleId))
