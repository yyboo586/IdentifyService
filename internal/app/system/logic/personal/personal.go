...
func (s *sPersonal) EditPersonal(ctx context.Context, req *system.PersonalEditReq) (user *model.LoginUserRes, err error) {
	...
	err = g.DB().Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
		err = g.Try(ctx, func(ctx context.Context) {
			_, err = dao.SysUser.Ctx(ctx).TX(tx).WherePri(userId).Update(map[string]interface{}{
				...
				dao.SysUser.Columns().UserEmail:    req.UserEmail,
				dao.SysUser.Columns().Describe:     req.Describe,
