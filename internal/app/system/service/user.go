...
type (
	IUser interface {
		...
		// 修改用户权限(组织管理员才能修改)
		EditUserPermission(ctx context.Context, req *system.EditUserPermissionReq) (err error)
		// 重置用户密码
		ResetUserPwd(ctx context.Context, req *system.UserResetPwdReq) (err error)

		// 通过id获取用户信息
		GetUserInfoByID(ctx context.Context, id string) (user *model.User, err error)
		// 获取用户列表
		...
		// 验证用户名和密码
		ValidateUsernameAndPassword(ctx context.Context, userName, password string) (user *model.User, err error)
	}
)
