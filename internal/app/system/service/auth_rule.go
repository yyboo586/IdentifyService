...
type (
	IAuthRule interface {
		...
		Get(ctx context.Context, id int64) (out *model.AuthRule, err error)
		List(ctx context.Context) (out []*model.AuthRule, err error)
		GetTree(ctx context.Context, id int64) (out *model.AuthRuleNode, err error)
		ListTree(ctx context.Context) (out []*model.AuthRuleNode, err error)

		// ========== 登录场景接口 ==========
		// 获取用户登录后的菜单树（目录+菜单，不包含按钮）
		GetMenuTreeByUserID(ctx context.Context, userID string) (out []*model.AuthRuleNode, err error)
		// 获取用户登录后的按钮权限列表
