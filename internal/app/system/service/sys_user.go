...
type (
	ISysUser interface {
		...
		CheckUserExistsByUserID(ctx context.Context, userID uint64) (exists bool, err error)
		Login(ctx context.Context, userID uint, password string) (user *model.LoginUserRes, err error)
		GetRoleIds(ctx context.Context, userId uint64, includeChildren ...bool) (roleIds []uint, err error)
	}
)
