...
func (r *role) DeleteByIDs(ctx context.Context, roleIDs []int64) (err error) {
	...
	err = g.DB().Transaction(ctx, func(ctx context.Context, tx gdb.TX) (err error) {
		...
		err = r.delRoleUser(roleIDs)
		if err != nil {
			err = fmt.Errorf("删除角色用户失败: %w", err)
			return
		}
...
func (r *role) GetRolesByUserID(ctx context.Context, userID string) (out []*model.Role, err error) {
	...
	g.Log().Info(ctx, "roleIDs: ", roleIDs)
	for _, v := range roleIDs {
		var role *model.Role
		role, err = r.Get(ctx, v)
		if err != nil {
			g.Log().Error(ctx, err)
			return
		}
...
func (r *role) delRoleUser(roleIDs []int64) (err error) {
	for _, v := range roleIDs {
		_, err = r.enforcer.RemoveFilteredNamedGroupingPolicy("g", 1, gconv.String(v))
		if err != nil {
			return
		}
	}
