...
func (s *sSysConfig) Add(ctx context.Context, req *system.ConfigAddReq, userId uint64) (err error) {
	err = g.Try(ctx, func(ctx context.Context) {
		...
		_, err = dao.SysConfig.Ctx(ctx).Insert(map[string]interface{}{
			dao.SysConfig.Columns().ConfigName:  req.ConfigName,
			dao.SysConfig.Columns().ConfigKey:   req.ConfigKey,
			dao.SysConfig.Columns().ConfigValue: req.ConfigValue,
			dao.SysConfig.Columns().ConfigType:  req.ConfigType,
			dao.SysConfig.Columns().CreateBy:    userId,
			dao.SysConfig.Columns().Remark:      req.Remark,
		})
...
func (s *sSysConfig) Edit(ctx context.Context, req *system.ConfigEditReq, userId uint64) (err error) {
	err = g.Try(ctx, func(ctx context.Context) {
		...
		liberr.ErrIsNil(ctx, err)
		_, err = dao.SysConfig.Ctx(ctx).WherePri(req.ConfigId).Update(map[string]interface{}{
			dao.SysConfig.Columns().ConfigName:  req.ConfigName,
			dao.SysConfig.Columns().ConfigKey:   req.ConfigKey,
			dao.SysConfig.Columns().ConfigValue: req.ConfigValue,
			dao.SysConfig.Columns().ConfigType:  req.ConfigType,
			dao.SysConfig.Columns().UpdateBy:    userId,
			dao.SysConfig.Columns().Remark:      req.Remark,
		})
