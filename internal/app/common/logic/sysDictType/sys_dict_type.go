...
func (s *sSysDictType) Add(ctx context.Context, req *system.DictTypeAddReq, userId uint64) (err error) {
	err = g.Try(ctx, func(ctx context.Context) {
		...
		_, err = dao.SysDictType.Ctx(ctx).Insert(map[string]interface{}{
			dao.SysDictType.Columns().DictName: req.DictName,
			dao.SysDictType.Columns().DictType: req.DictType,
			dao.SysDictType.Columns().Status:   req.Status,
			dao.SysDictType.Columns().CreateBy: userId,
			dao.SysDictType.Columns().Remark:   req.Remark,
		})
...
func (s *sSysDictType) Edit(ctx context.Context, req *system.DictTypeEditReq, userId uint64) (err error) {
	err = g.DB().Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
		err = g.Try(ctx, func(ctx context.Context) {
			...
			//修改字典类型
			_, e = dao.SysDictType.Ctx(ctx).TX(tx).WherePri(req.DictId).Update(map[string]interface{}{
				dao.SysDictType.Columns().DictName: req.DictName,
				dao.SysDictType.Columns().DictType: req.DictType,
				dao.SysDictType.Columns().Status:   req.Status,
				dao.SysDictType.Columns().UpdateBy: userId,
				dao.SysDictType.Columns().Remark:   req.Remark,
			})
			liberr.ErrIsNil(ctx, e, "修改字典类型失败")
			//修改字典数据
			_, e = dao.SysDictData.Ctx(ctx).TX(tx).Data(map[string]interface{}{
