...
func (s *sSysDictData) Add(ctx context.Context, req *system.DictDataAddReq, userId uint64) (err error) {
	err = g.Try(ctx, func(ctx context.Context) {
		_, err = dao.SysDictData.Ctx(ctx).Insert(map[string]interface{}{
			dao.SysDictData.Columns().DictSort:  req.DictSort,
			dao.SysDictData.Columns().DictLabel: req.DictLabel,
			dao.SysDictData.Columns().DictValue: req.DictValue,
			dao.SysDictData.Columns().DictType:  req.DictType,
			dao.SysDictData.Columns().CssClass:  req.CssClass,
			dao.SysDictData.Columns().ListClass: req.ListClass,
			dao.SysDictData.Columns().IsDefault: req.IsDefault,
			dao.SysDictData.Columns().Status:    req.Status,
			dao.SysDictData.Columns().CreateBy:  userId,
			dao.SysDictData.Columns().Remark:    req.Remark,
		})
...
func (s *sSysDictData) Edit(ctx context.Context, req *system.DictDataEditReq, userId uint64) (err error) {
	err = g.Try(ctx, func(ctx context.Context) {
		_, err = dao.SysDictData.Ctx(ctx).WherePri(req.DictCode).Update(map[string]interface{}{
			dao.SysDictData.Columns().DictSort:  req.DictSort,
			dao.SysDictData.Columns().DictLabel: req.DictLabel,
			dao.SysDictData.Columns().DictValue: req.DictValue,
			dao.SysDictData.Columns().DictType:  req.DictType,
			...
			dao.SysDictData.Columns().IsDefault: req.IsDefault,
			dao.SysDictData.Columns().Status:    req.Status,
			dao.SysDictData.Columns().UpdateBy:  userId,
			dao.SysDictData.Columns().Remark:    req.Remark,
		})
