...
// UserSearchReq2 用户搜索请求参数
type UserSearchReq2 struct {
	g.Meta   `path:"/user/list2" tags:"系统后台/用户管理" method:"get" summary:"用户列表"`
	DeptId   string `p:"deptId"` //部门id
	RoleId   uint   `p:"roleId"`
	Mobile   string `p:"mobile"`
	Status   string `p:"status"`
...
type UsersRoleNameReq struct {
	g.Meta   `path:"/user/getByRoleName" tags:"系统后台/用户管理" method:"get" summary:"获取角色对应用户"`
	RoleName string `json:"role_name" v:"required" dc:"角色名称"`
}
type UsersRoleNameRes struct {
	commonApi.EmptyRes
	UserList []*model.SysUserRoleDeptRes `json:"userList"`
}
