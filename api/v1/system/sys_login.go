...
type DeleteUserDeviceReq struct {
	g.Meta `path:"/user/{user_id}/device/{device_id}" tags:"用户设备管理" method:"delete" summary:"删除用户设备"`
	model.Author
...
type ListUserDeviceReq struct {
	g.Meta `path:"/user/{user_id}/device" tags:"用户设备管理" method:"get" summary:"获取用户设备列表"`
	model.Author
	UserID string `p:"user_id" v:"required#用户ID不能为空"`
	model.PageReq
}
